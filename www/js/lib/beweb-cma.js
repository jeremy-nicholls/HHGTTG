...
// 20 jun 2007 - added WriteFieldEx and WriteTextFieldEx
// 20 jun 2007 - now uses global variable df_attachmentPPath to handle disabled parent paths
// 20 jun 2007 - CropImageWithin now works with non-square target shapes (cximage only so far)
// 13 jun 2007 - ensure user is logged in for all types of request - increased security
// 17 may 2007 - added back button prevention for new records
// 03 may 2007 - shows waitbox when saving
// 03 may 2007 - disallows uploading of executable file extensions
// 03 may 2007 - fixed bug with AspSmartUpload subform upload
// 01 may 2007 - added in AspJpeg image resizing support
// 05 mar 2007 - added back button trap code, not integrated live yet though
// 01 mar 2007 - supports df_returnpage=closewindow and df_returnpage=javascript:whatever
// 28 feb 2007 - scalimage keep aspect ratio on smaller thumbs
//  9 Jan 2007	- made subforms javascript work in Firefox
// 19 dev 2006 - made subforms work with file uploads
// 15 dec 2006 - does not delete attachment if same file is referenced twice
// 12 dec 2006 - added df_CancelFormSubmit js function to stop form being submitted if a callback validation fails
// 20 nov 2006 - ensure can't postback twice (browser can take a second or so before actually posting form)
// 06 nov 2006 - CropImageWithin: if no prefix, dont include underscore
// 18 oct 2006 - added function for duplicating records
// 10 oct 2006 - subform where clause can now also contain 'order by' clause
// 29 aug 2006 - VPATHMARKERText only replace if exists - for backward compatibility
// 08 aug 2006 - now allows spaces in field names (great!), added WritePasswordField, fixed encrypted pk deletion
// 21 jul 2006 - improvements to subforms
// 06 jul 2006 - new algorithm for SaveCategoriesEx
// 06 Jul 2006 - JJN - added ProcessAttachmentLocationMarkers
// 30 Jun 2006 - Added CropImageWithin
// 29 Jun 2006 - JJN - added error check for upload comp fail
// 29 jun 2006 - now keeps user in number field until it is valid
// 08 jun 2006 - added support for df checkboxes to df_SetFieldReadOnly, added id to row del button
// 16 may 2006 - added GetRowSuffixArray, other fixes
// 19 apr 2006 - changed ScaleImageWithin to save out prefixed version even when no scaling required; made SAImageGen last choice
// 20 mar 2006 - save button now ensures callbacks have completed before submitting form
//             - added limitlength check to onchange in textfield
// 03 mar 2006 - got stuff from pengellys
// 19 jan 2006 - df_WriteSubformWithRecordset now moves to first record
// 21 dec 2005 - improved image quality settings for SAImageGen
//             - callbacks now work without subforms
// 01 dec 2005 - added df_showSubformDeleteButton, df_isAddingRow and df_WriteSubformWithRecordset
// 29 nov 2005 - df_CheckSubforms: fixed bug where it tried to validate deleted rows
// 21 nov 2005 - added new fn df_GetImageSizingComponentName(), called by ScaleImageWithin
// 21 nov 2005 - ScaleImageWithin - return maxWidth and maxHeight modified.
// 10 nov 2005 - checks for EOF in case of rs.CancelUpdate or rs.Delete in subform save
//             - callbacks now display javascript errors as alert box
//             - df_SetFieldReadOnly: new javascript function to use instead of field.disabled
//             - df_GetFieldsInAllRows, GetSubformTotal: swapped parameters around
//             - df_GetFieldInSameRow: added validation
// 10 nov 2005 - added CxImageATL.CxImage support to ScaleImageWithin
// 10 nov 2005 - added support for SAImageGen in ScaleImageWithin
// 09 nov 2005 - ScaleImageWithin: put on error resume next around file delete
// 03 nov 2005 - added df_EnableField javascript functions, df_numberFieldMaxPixelWidth, df_numberFieldAlign
// 27 oct 2005 - added javascript functions for getting and setting dropdown values
// 18 oct 2005 - added utility function df_GetSubformTotal()
// 13 oct 2005 - fixed multiple subforms, fixed bug when rs.addnew on empty rs using 'order by', 
//							- added df_doExtraUpdate, sync codelib and pengellys
// 06 oct 2005 - added df_warning
// 20 sep 2005 - sync codelib and pengellys, subforms changed to double underscores, uniqueLabel removed from W/riteCategoryCheckboxes/for backwards compatibility
// 06 sep 2005 - added out, dout / also added WriteCategoryCheckboxesEx and savecategoriesex to handle unique labels
// 23 aug 2005 - added subformName param to df_AddSubformEx()
// 04 aug 2005 - changed subforms to use double underscores
// 03 aug 2005 - added df_allowCallbacks which defaults to false under SSL
// 11 jul 2005 - fixed js error on down arrow, fixed encrypted id, and set db type
// 07 jul 2005 - added cf_OnDeleteXXX custom event when deleting a row
// 27 jun 2005 - added subform where clause
// 08 jun 2005 - new subforms feature (eg invoice header and lines)
//             - support for client-to-server callbacks
//             - df_isDebug flag
//             - df_isNewRecord no longer changes during the add process
// 20 may 2005 - made image resizing prefix work with pathnames in the filename field
// 30 apr 2005 - added cf_OnChange<fieldname> to allow hooking into field changes
// 30 apr 2005 - added df_TrimValues, df_useSubmitButton
// 22 apr 2005 - sets focus back to posted back field if supplied
// 30 mar 2005 - added "where 0=1" when creating recordset for adding
// 07 mar 2005 - added df_returnpage check for blank breadcrumb
//             - made it not write save button if df_WriteSaveButton is passed empty string
// 25 feb 2005 - split df_SaveRecord into df_SaveRecordValues and df_SaveRecordUpdate
//             - added df_wasNewRecord
//             - added modification log with df_useModificationLog switch
// 21 feb 2005 - fixed javascript for delete, postback etc for mozilla browsers
// 14 feb 2005 - added df_maxPixelWidth for controls
// 11 feb 2005 - added WriteNumberField with validation and currency formatting
// 04 sep 2004 - added df_isEncryptedPK which decrypts the ID when it is passed in
// 15 aug 2004 - added WriteTextField and df_postbackfield
// 02 aug 2004 - returnpage now handles blank referer for norton firewall
// 16 jul 2004 - added df_ShowDetailedFatalErrors and df_StripNumerics
// 04 apr 2004 - added ScaleImageWithin to general helper functions
// 27 feb 2004 - supports Persits AspUpload
// 21 jan 2004 - introduced df_error for error message handling; also added df_mode
// 15 jan 2004 - supports Dundas Upload component
//             - added df_BeforeSubmit javascript function to send fieldnames
//             - unique filenames were not working properly if too long
// 22 sep 2003 - supports MySQL databases
// 12 sep 2003 - added up_maxFileSize global to stop huge files being uploaded
// 05 sep 2003 - fixed non-numeric pk bug
// 29 aug 2003 - added WriteTextField
// 15 Jul 2003 - fixed bug in filename length check which was removing the extension
// 02 Jun 2003 - removed filetype check in aspSmartUpload
// 04 May 2003 - fixed bug when calling safileup isMissing
// 23 Apr 2003 - added filename length check
// 22 Apr 2003 - improved saFileup null handling and cleaner filenames
// 16 Apr 2003 - added support for ActiveFile and autocheck
// 21 Mar 2003 - fixed bug in DeleteCategories
// 19 Mar 2003 - added domain license check
// 12 Mar 2003 - added general helper routines
// 13 Feb 2003 - converted from inline ASP script to runat=server script block
// 22 Oct 2002 - added df_isNumericPK, df_databaseType, df_sql
// 28 Aug 2002 - handles non-numeric primary keys again
// 28 Aug 2002 - now always uses updateable recordset when displaying
// 16 Aug 2002 - Pengellys, re-activated cf_Postback call
// 01 Aug 2002 - added Postback feature
// 22 Nov 2001 - major upgrade - integrated file upload code; prefixed all names with df_; transaction support
// 31 Aug 2001 - changed some routine names to avoid clashes
// 23 Aug 2001 - added file upload code
// 07 Aug 2001 - allows setting of df_request for file uploads
// 23 May 2001 - handles non-numeric primary keys
// 16 May 2001 - ensures ADOVBS.INC is included
...
function E$(element) {
	...
	if (!element) {
		alert("Element ID not found: " + element)
	}
	return element;
}
...
function df_AddRow(subformCode) {
	...
	var res = df_InsertRow(subformCode, -1);
